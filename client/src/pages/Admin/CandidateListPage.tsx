...
const CandidateListPage = ({ setEditCandidate }: any) => {
  ...
  useEffect(() => {
    if (accessToken) {
      dispatch(getAllCandidate(accessToken));
    }
  ...
  const columns = React.useMemo<ColumnDef<any>[]>(
    () => [
      ...
      {
        accessorFn: (row) => row.CandidteName,
        id: "CandidteName",
      ...
      },
      {
        ...
        cell: (info) => (
          <div>
            <div>
              {info.row.original.PlaceOfBirth},{" "}
              {moment(info.row.original.DateOfBirth).format("DD-MM-YYYY")}
            </div>
          </div>
        ),
        header: () => <span>Tempat dan Tanggal Lahir</span>,
        footer: (props) => props.column.id,
      },
      {
        accessorFn: (row) => row.CandidatePosition,
        id: "CandidtePosition",
        cell: (info) => info.getValue(),
        header: () => <span>Posisi Yang dilamar</span>,
        footer: (props) => props.column.id,
      },
      {
        accessorFn: (row) => row.education[0]?.Major,
        id: "endidikan Terakhir",
        cell: (info) => info.getValue(),
        header: () => <span>Pendidikan Terakhir</span>,
        footer: (props) => props.column.id,
      },
      {
        id: "action",
